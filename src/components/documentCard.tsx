...
export const DocumentCard = ({ document, isLink }: DocumentCardProps) => {
    const imageSrc = useMemo(() => {
        ...
        return hasValidImage ? `/pages/${firstPage.image.url}` : '/images/document-placeholder.png';
    }, [document.pages, document.excel_type]);

    return (
        <Card
            className={cn('w-[300px]', {
                'cursor-pointer transition-all duration-200 hover:shadow-lg hover:-translate-y-1': isLink,
            })}
        >
